...
function customAssign(...objects) {
  ...
  const arr1 = Object.keys({ ...objects }).length > 2;
  if (arr1 === !true) {
  ...
  let element;
  for (element in objAssign) {
    // якщо значення типу число конвертуємо його в рядок
    let elementObj = objAssign[element];
    if (typeof elementObj == "number") {
      objAssign[element] = elementObj.toString();
    }
  }
...
function customObjectFromEntries(entries) {
  ...
  const arr1 = Array.isArray(entries);
  if (arr1 === !true) {
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  const arr1 = typeof obj1 === "object" && typeof obj2 === "object";
  if (arr1 === !true) {
    return null;
  }
...
function keysAndValues(obj) {
  ...
  return Array.of(objKeys, objValues);
}
...
function filterObjectsByKey(arr, key) {
  ...
  const arr1 = Array.isArray(arr);
  if (arr1 === !true) {
    return [];
  }
  // Використовуємо метод filter() для вибірки об'єктів, що містять вказаний ключ
  // const arrFilter = arr.filter((element) => element.hasOwnProperty(key));
  const arrFilter = arr.filter((element) => key in element);
...
function checkOwnership(obj1, obj2, key) {
  ...
  const hasPropertyNo = obj1.hasOwnProperty(key) && obj2.hasOwnProperty(key);
  if (hasPropertyNo == false) {
    return false;
  }
}
...
function getObjectValuesSum(obj) {
  ...
  const objSumm = objValues.reduce((item, itNumber) => item + itNumber);
  // Повертаємо суму
  return objSumm;
}
